...
{ 
	$errors = array();
	$idCommentToValidate = htmlspecialchars($_GET['id']);
	...
	$idCommentToValidate = (int) $idCommentToValidate;
	...
	if (isset($idBlog) AND !empty($idBlog) AND $idBlog == $idDataBase)
	{
		if (isset($auteurComment) AND !empty($auteurComment))
		{
			if (isset($message) AND !empty($message))
			{
				...
				$birth = $CommentManager->birth($Comment, $idBlog);

				if ($birth == true)
				{
					$delete = $CommentManager->delete($idCommentToValidate); 
				}
				else
				{
